...
use encryption::{xor, ENCRYPTION_KEY};
use peer_node;
...
pub fn start_node_connection(
    ...
    thread::spawn(move || {
        match ping_to_master(cloned_local_node_for_ping_pong, cloned_peer_nodes) {
            Ok(_) => {}
            Err(_e) => {}
        }
    });
        ...
        if let Some(peer_node) = locked_peer_nodes.get_mut(&replica_addr) {
            match peer_node.stream.try_clone() {
                Ok(peer_stream) => {
                    ...
                    let _ =
                        serialize_vec_hashmap(&cloned_shared_documents, peer_stream.try_clone()?);
                }
                Err(_) => {
                    eprintln!("Error cloning stream");
                }
...
fn serialize_vec_hashmap(
    ...
    for (key, doc) in map {
        ...
        let encrypted_message = xor(line.as_bytes(), ENCRYPTION_KEY);
        stream.write_all(&encrypted_message)?;
    }
...
fn initialize_replica_promotion(
    ...
    println!("initializing replica promotion");
    {
        ...
        let encrypted_node_message = xor(node_info_message.as_bytes(), ENCRYPTION_KEY);

        let inactive_node_message = format!("inactive_node {}\n", inactive_port);
        let encrypted_inactive_message = xor(inactive_node_message.as_bytes(), ENCRYPTION_KEY);
