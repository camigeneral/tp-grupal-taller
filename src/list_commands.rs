#[allow(dead_code)]
pub fn lset(list: &mut Vec<String>, index: i32, element: &str) {
    let mut index_usize = 0;
    if index < 0 {
        let abs_index = index.abs() as usize;
        index_usize = list.len() - abs_index;
    } else {
        index_usize = index as usize;
    }
...
#[allow(dead_code)]
pub fn llen(list: &Vec<String>) -> usize {
    list.len()
}
...
mod tests {
    ...
    fn test_linsert() {
        let mut list: Vec<String> = vec!["1".to_string(), "2".to_string(), "3".to_string()];
        linsert(&mut list, "before".to_string(), "2".to_string(), "4".to_string());
        assert_eq!(list[0], "1");
        assert_eq!(list[1], "4");
        assert_eq!(list[2], "2");
        assert_eq!(list[3], "3");

        linsert(&mut list, "after".to_string(), "2".to_string(), "5".to_string());
        assert_eq!(list[0], "1");
        assert_eq!(list[1], "4");
        assert_eq!(list[2], "2");
        assert_eq!(list[3], "5");
